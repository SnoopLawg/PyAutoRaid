...
class CBSimulator:
    ...
    def _champion_turn(self, champ: SimChampion, tick: int):
        ...
        if chosen.multiplier > 0 and chosen.hit_count > 0:
            ...
            # DestroyHp / HP destruction is NO-OP vs Demon Lord (game
            # mechanic): the CB boss "Infernal Resilience" passive makes
            # it immune to MAX HP reduction effects. Verified via
            # community sources 2026-06-22 (user correction). Static
            # skill 62801 (Venom A1) has 2x DestroyHp(0.75*DEALT_DMG)
            # which fires in dungeons/other content but NOT vs CB.
            # The earlier impl that attributed DestroyHp to Venom on CB
