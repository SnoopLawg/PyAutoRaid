...
| Interactive visual timeline | ✅ 2026-06-25 | DWJ-style turn-meter timeline in the TuneLab cast panel: per-action grid (rows = casts, columns = each champion's TM fill bar), acting champ highlighted green / boss violet, amber dot = speed buff/debuff active, with a legend + tooltips. Backend: `calc_parity_sim.simulate(capture_state=True)` attaches a **read-only** per-turn snapshot (`state.tm` + `state.speed`) — gated so the verified-parity path is byte-identical (both `test_scheduler_parity` tests green); `dashboard_server` serializes `state`/`actor_order`/`tm_max`. Frontend falls back to the old text list if `state` is absent (old cached responses). Faithful to DWJ's actual scope (scheduler tracks TM + speed effects only; UK/BD aren't in DWJ's calc either). |
...
| A4 diagnosis (precise, exact-stats) | 🟡 | **Two baseline-MEN Force runs bracket the real distribution**: run1 `052744` wiped **T24** (bad glances; Ninja 5% HP by T19), run2 `054030` wiped **T35** (good glances; 5/5 alive through T32). So real Force survival is a **WIDE distribution (~T24–35)** driven by glance RNG, NOT a fixed number. The sim doesn't reproduce the shape: **deterministic = T34 (too optimistic — matches only the good tail); MC (16 seeds, bbt=False) = T23–26, clustered at 23 (too pessimistic AND too tight — never produces the good-luck T35 tail)**. Root: the glance model's BINARY full-TM-fill-zeroing is mis-shaped (deterministic averages it away; MC over-punishes). **A4 fix target = make the MC survival distribution match real (~T24–35 spread)** — and since glancing only fires on weak-affinity (Force for this team), calibrating it is Force-legitimate and won't touch stable-day survival. **But 2 points is too thin to fit a distribution** — needs more Force survival samples (future keys) before re-shaping the glance model; NOT done blindly. |
...
| Sub-goal | Status | Notes |
|---|---|---|
| B1 — Coverage-chain enumerator | ✅ 2026-06-25 | `tools/cb_comp_finder.py` — from `data/m5_synergy.jsonl` + owned roster, enumerates candidate comps per archetype (uk / bd / counter / protect / heal / revive) with the provider core + DPS fillers, scored by game-truth synergy coverage (**no HellHades** in the path — unlike `cb_team_explorer`). Survival is explicitly labeled UNVALIDATED (it generates candidates; the verdict is Gate B). `--validate` runs cb_sim survival but warns it's optimistic until Gate A. **Synergy-data gap CLOSED 2026-06-25**: `buff_extension` is now a tagged `provides` axis (effect kind `IncreaseBuffLifetime`, e.g. Demytha A2) and defensive-buff providers (UK/BD/Shield/Cont.Heal/Counter) carry a matching `needs:buff_extension`, so BD+extension stall chains are now enumerable. 44 providers tagged. |
...
| B3 — Auto-vs-manual classifier | ✅ 2026-06-25 | `classify_cast_pattern()` in `cb_comp_finder.py`: a **structural** (Gate-A-independent) test of whether the archetype's defensive buff can be kept GAPLESS by a static preset (opener+priority = cast-on-cooldown). Game-truth inputs: per-skill cooldown + buff duration (`skills_db`, book-accurate) + the `buff_extension` axis + multi-provider stagger. **AUTO** iff buff dur≥cd, or A1/permanent, or an extension ally re-ups it, or 2+ providers stagger; else **MANUAL** (needs a timed hold a preset can't express — never auto-recommended; `--auto-only` filters them out). Verified: Maneater UK (dur2/cd7) + Demytha extension = AUTO (matches the real reliable MEN tune); Uugo BD (dur1/cd5, no extender) = MANUAL. NOTE: this classifies the *cast cadence*, not survival — an AUTO comp can still wipe (that's Gate A/B). |
