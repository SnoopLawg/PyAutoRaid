...
| Sub-goal | Status | Notes |
|---|---|---|
| **Generalized gear optimizer (per-champion stat targets)** | 🟡 2026-06-23 — **shipped** `tools/gear_target_optimizer.py`. Per-stat min/max/importance + modes (balanced/damage/survivability), any champion, location-agnostic. Lexicographic min-satisfaction + **set-aware seeding** (single-slot hill-climb can't cross a 2/4-piece set threshold; seeds force-fill stat-relevant sets). Uses `calc_stats` as the oracle (game-accurate: Venom ceiling 162 SPD == her in-game 162). Verified: meets achievable mins, honest MISS when unachievable, damage-mode caps CR at min. **Still TODO**: team-wide optimization with cross-hero locking (HH does multi-champion); accessory primary constraints. |
