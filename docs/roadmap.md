...
| Recommender per location | 🟡 2026-06-23 — `tools/m5_recommender.py` (greedy axis-coverage team builder, 8 location profiles, CC downweighted vs CC-immune bosses, game-truth ACC floors surfaced) + `tools/m5_roster_gaps.py` ("what to pull/build next" per location — hard gaps, thin axes, best unowned HH-rated upgrades). Still needs sim-validation of recommended teams + per-build damage targets. |
