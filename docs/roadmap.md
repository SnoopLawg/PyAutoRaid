...
| Recommender per location | 🟡 2026-06-23 — `tools/m5_recommender.py` (greedy axis-coverage team builder, 8 location profiles, CC downweighted vs CC-immune bosses, game-truth ACC floors; `--builds` adds per-hero builds) + `tools/m5_roster_gaps.py` (pull/build priorities) + `tools/m5_build_recommender.py` (per-hero masteries/blessing/stat focus, grounded in the hero's game-truth kit + per-location relevance). Still needs sim-validation of recommended teams + per-build CR/CD/ATK% damage targets. |
